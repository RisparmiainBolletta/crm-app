...
export function collegaPulsantiModifica() {
    document.querySelectorAll(".btn-modifica").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            document.getElementById("mod-iban").value = btn.dataset.iban || "";
            document.getElementById("mod-tipo-richiesta").value = btn.dataset.tipo_richiesta || "";
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", function (event) {
        ...
        const datiAggiornati = {
            ...
            IBAN: document.getElementById("mod-iban").value,
            Tipo_richiesta: document.getElementById("mod-tipo-richiesta").value,
        };

        const urlBase = window.location.pathname.includes("admin") ? "/admin/clienti" : "/clienti";

        const iban = document.getElementById("mod-iban").value;
        if (iban && !validaIBAN(iban)) {
            alert("❌ IBAN non valido. Deve essere un IBAN italiano valido.");
            return;
        }
